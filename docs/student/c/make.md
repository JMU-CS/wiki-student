...
The first description block says that the target `test` depends on two files, `test.o` and `module1.o`. If `test` has an earlier time stamp than either of these files, `test` should be rebuilt using the [g++ compiler](gpp.md).
...
The make command uses a number of predefined/implicit [rules](http://www.gnu.org/software/make/manual/html_chapter/make_10.html#SEC105) that can greatly simplify makefiles. For example, `.o` files will automatically be created from `.c` or `.cpp` files using the GCC.
...
To get a description of an error or warning message, see the comprehensive list of [errors generated by make](http://www.gnu.org/software/make/manual/make.html#Error-Messages).
